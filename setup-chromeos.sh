...
print_header() {
    ...
    echo -e "${BLUE}  (Chrome OS Flex Edition)${NC}"
    echo -e "${BLUE}======================================${NC}"
    echo ""
}
...
# Check if running in Crostini (Linux development environment)
check_crostini() {
    if [ ! -d /mnt/chromeos ]; then
        print_error "Linux development environment not detected."
        echo ""
        echo "Please enable Linux (Beta) first:"
        echo "  1. Open Settings"
        echo "  2. Go to 'Advanced' > 'Developers'"
        echo "  3. Turn on 'Linux development environment'"
        echo "  4. Follow the setup prompts"
        echo "  5. Re-run this script inside the Linux terminal"
        exit 1
    fi
    print_success "Running in Chrome OS Linux environment (Crostini)"
}
...
install_docker() {
    ...
    echo ""
    read -p "Press Enter to continue (you may need to re-run this script after logging back in)..."
}
...
check_docker() {
    ...
    if ! docker info &> /dev/null; then
        if sudo docker info &> /dev/null; then
            print_warning "Docker is running but requires sudo."
            echo "You may need to log out and log back in for group membership to take effect."
            echo ""
            read -p "Continue with sudo? [Y/n]: " USE_SUDO
            USE_SUDO=${USE_SUDO:-Y}
            if [[ ! "$USE_SUDO" =~ ^[Yy]$ ]]; then
                echo "Please log out, log back in, and run this script again."
                exit 1
            fi
            DOCKER_CMD="sudo docker"
            DOCKER_COMPOSE_CMD="sudo docker compose"
        else
            print_error "Docker daemon is not running. Please check Docker installation."
            exit 1
        fi
    else
        DOCKER_CMD="docker"
...
# Auto-generated by setup-chromeos.sh
...
setup_hosts() {
    ...
    echo "(This modifies the Linux container's /etc/hosts, not Chrome OS)"
    echo ""
...
build_and_start() {
    ...
    if [[ "$BUILD_NOW" =~ ^[Yy]$ ]]; then
        ...
        echo ""
        echo "From Chrome browser (recommended):"
        for i in $(seq 1 $INSTANCES); do
            LETTER=$(echo $i | awk '{printf "%c", 96+$1}')
            PORT=$((BASE_PORT + i - 1))
            echo "  - Instance $LETTER: http://penguin.linux.test:$PORT"
        done
        echo ""
        echo "From Linux terminal:"
        for i in $(seq 1 $INSTANCES); do
            LETTER=$(echo $i | awk '{printf "%c", 96+$1}')
            PORT=$((BASE_PORT + i - 1))
            echo "  - Instance $LETTER: http://localhost:$PORT or http://rstudio-$LETTER:$PORT"
        done
        echo ""
        echo "Credentials:"
        for i in $(seq 1 $INSTANCES); do
            LETTER=$(echo $i | awk '{printf "%c", 96+$1}')
            echo "  - Instance $LETTER: rstudio_$LETTER / rstudio_$LETTER"
        done
        echo ""
